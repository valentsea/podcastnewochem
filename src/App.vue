...
<template>
  ...
  <div class="container">
    <div class="page">
      <div class="left-block">
        ...
        <div class="listen" v-show="!isMinimizedInfo">
          <div class="listen__review">
            <addSVG icon="arrow-right-up" /><span> Оставьте нам отзыв</span>
          </div>
        ...
        >
          <div class="playlist">
            ...
            <div class="playlist__tracklist">
              <template v-if="playlist.length != 0">
                <draggable v-model="playlist" :sort="true" handle=".handle">
                  <transition-group>
                    <template v-for="track in playlist" :key="track.id">
                      <div
                        :class="{ playing: zPlayer.id == track.id }"
                        class="playlist__track playlist-track"
                      >
                        <div
                          class="playlist-track__play"
                          @click="playEpisode(track.id)"
                        >
                          <addSVG
                            icon="play"
                            :showIconIf="
                              zPlayer.id != track.id || !this.zPlayer.isPlaying
                            "
                          />
                          <addSVG
                            icon="pause"
                        ...
                        <div class="playlist-track__handle handle">
                          <addSVG icon="drag" />
                        </div>
                        ...
                        <div
                          ...
                          @click="toggleTrackInPlaylist(track)"
                        >
                          <addSVG icon="close" />
                        </div>
                      </div>
              ...
              <div class="playlist__empty-pllst" v-if="playlist.length == 0">
            ...
            <div class="playlist__bottom">
              ...
              <span class="playlist__make-empty" @click="togglePlaylist">
                Выключить плейлист
              </span>
            </div>
          </div>
      ...
      <div class="tracks">
        <div class="tracks__head topline" v-if="showPlaylists || showEpisodes">
          <button v-if="!isMobile" @click="togglePlaylistsShow" class="switch">
            <span
              class="switch__btn"
            ...
            >
              ...
              <sup>
                ...
                <template v-else>{{ episodes.length }}</template></sup
              >
            </span>
            ...
            <span
              ...
              :class="{ switch__btn_active: !showEpisodes }"
            >
              Подборки <sup> {{ plylsts.length + userPlaylists.length }} </sup>
            </span>
          </button>
          <template v-if="showEpisodes">
            <div class="topline__left" v-if="isMobile">
              <div class="topline__header">
                ...
                <span class="topline__number">
                  ...
                  <template v-else>{{ episodes.length }}</template>
                </span>
              </div>
            </div>
            <input
            ...
            <div class="topline__icons">
                ...
                <div class="topline__sort-block">
                  <div @click.stop="sortByDate">
                    ...
                    <span class="topline__sort-arrow">
                      <addSVG
                        :showIconIf="sortDateDESC === false"
                        icon="sort-up"
                      />
                      <addSVG
                        :showIconIf="sortDateDESC === true"
                        icon="sort-down"
                      />
                    </span>
                  </div>
                  ...
                  <div @click.stop="sortByDuration">
                    ...
                    <span class="topline__sort-arrow">
                      <addSVG
                        :showIconIf="sortDuration === true"
                        icon="sort-up"
                      />
                      <addSVG
                        :showIconIf="sortDuration === false"
                        icon="sort-down"
                      />
                    </span>
                  </div>
              ...
              <div class="topline__search-icon" @click="toggleSearch()">
                <addSVG icon="search" :showIconIf="!isSearchActive" />
                <addSVG icon="close" :showIconIf="isSearchActive" />
              </div>
            </div>
          </template>
          <template v-if="showPlaylists && isMobile"> Подборки </template>
        </div>
        <div class="tracks__buttons">
          ...
          <button @click="createCustomPlaylist" v-if="showEpisodes">
            <span class="random-episode__text"> Создать плейлист </span>
          </button>
        </div>
        <div
        ...
        >
          ...
          <template v-for="(plst, index) in userPlaylists" :key="plst.name">
            <div
              class="plylst"
              @click="playPlylst(plst.episodes, plst.name)"
              :style="{ backgroundImage: 'url(' + plst.img + ')' }"
            >
          ...
          <template v-for="plst in plylsts" :key="plst.name">
            ...
            >
              ...
              <div class="plylst__name">{{ plst.name }}</div>
              <div class="plylst__tooltip" style="background: #392562">
                Подборка редакции
              </div>
            </div>
          </template>
        </div>
        <template v-if="podcastDisplayedEpisodes.length && showEpisodes">
          <div
          ...
          >
            ...
            >
              <addSVG
                icon="play"
                :showIconIf="zPlayer.id != item.id || !zPlayer.isPlaying"
              />
              <addSVG
                icon="pause"
                :showIconIf="zPlayer.id == item.id && zPlayer.isPlaying"
            ...
            <div class="episode__main">
              ...
              <div class="episode__subline subline">
                ...
                >
                  ...
                  <span class="nowrap">
                    <template v-if="playlistActive && !playlist.includes(item)">
                      Добавить в плейлист
                    </template>
                    <template v-if="playlistActive && playlist.includes(item)">
                      Убрать из плейлиста
                ...
                <span
                  ...
                  :class="{
                    episode__about_active: openedDescriptions.includes(item.id),
                  }"
                >
                  <addSVG icon="hashtag" />
            ...
            <div class="episode__menu">
              ...
              <div class="episode__submenu">
                ...
                <a
                  ...
                  class="episode__download button"
                >
                  <addSVG icon="download" /><span>Скачать</span>
                </a>
              </div>
            </div>
          </div>
        ...
        <div class="zPlayer__main-block">
          <div class="zPlayer__main-left">
            <div class="zPlayer__play" @click="zPlayerToggle">
              <addSVG icon="play" :showIconIf="!this.zPlayer.isPlaying" />
              <addSVG icon="pause" :showIconIf="this.zPlayer.isPlaying" />
            </div>
            <div
              class="zPlayer__next"
              :class="{
...
export default {
  ...
  watch: {
    ...
    userPlaylists: {
      handler(newUserPlaylists, oldUserPLaylist) {
        localStorage.setItem("userPlalists", JSON.stringify(newUserPlaylists));
      },
      deep: true,
    },
  ...
  methods: {
    ...
    playNextTrackFromPlaylist() {
      const index = this.playlist.findIndex((n) => n.id == this.zPlayer.id);
